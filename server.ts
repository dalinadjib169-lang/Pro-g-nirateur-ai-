...
async function startServer() {
  ...
  app.post("/api/generate", async (req, res) => {
    try {
      ...
      const { generationType, teacherInfo, subjectInfo, aiPrompt, documentLanguage, includeImages, includeWatermark, contentStyle, designStyle, pageFrame } = req.body;
      ...
      7. **الرموز الرياضية والمتغيرات**: في العمليات الحسابية والرياضيات والفيزياء، استخدم الحروف اللاتينية حصراً (x, y, z, a, b, c) ولا تستخدم الحروف العربية (مثل س، ص، ع) أبدأ في المعادلات.
      8. أنت خبير في إعداد المذكرات التربوية الجزائرية. أنشئ مذكرة احترافية ومنظمة وجذابة.
      ...
      5. **الأشكال والرسومات**:
         ${includeImages 
           ? `المستخدم فعل خيار (إدراج الصور والرسوم / مذكرة بصور). أضف عند الحاجة فقط: رسوماً تعليمية مرتبطة مباشرة بالدرس (عبر SVG أو إيموجي مناسبة)، أيقونات بجانب بعض العناوين، صوراً أو رسومات توضيحية تساعد على الفهم، وعناصر بصرية مناسبة لعمر التلاميذ. اجعل الرسوم منسجمة مع المادة (الرياضيات: أشكال هندسية، العلوم: حيوانات/نباتات، الجغرافيا: خرائط، التاريخ: آثار، اللغات: كتب/أقلام). لا تضف صوراً إذا لم تكن مفيدة للمحتوى، ولا تكررها بلا داعٍ.`
           : `المستخدم فعل خيار (مذكرة عادية / بدون صور). لا تضف أي صور أو رسوم تعليمية أو أيقونات (باستثناء أيقونات التصميم الأساسية جداً)، واكتفِ بمذكرة احترافية منسقة بالنصوص والجداول والألوان.`
         }
